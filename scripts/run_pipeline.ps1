param(
  ...
  [ValidateSet("tesseract", "mineru")][string]$Engine = "tesseract",
...
if ($Engine -eq "tesseract") {
  ...
  Write-Host "[1/3] Running OCR extraction (tesseract)..."
...
Write-Host "[1/2] Running MinerU extraction..."
python "u:\2026src-ni\scripts\extract_pdf_mineru.py" `
  --pdf "$PdfPath" `
  --out "$OutDir" `
  --mineru-runner "uv run mineru" `
  --backend "pipeline" `
  --clean
if ($LASTEXITCODE -ne 0) {
  throw "Step 1 failed: MinerU extraction"
}

Write-Host "[2/2] Done"
Write-Host "- Normalized Markdown: $OutDir\ocr_full.md"
Write-Host "- Normalized JSON: $OutDir\structured\summary.json"
Write-Host "- Run manifest: $OutDir\mineru_manifest.json"
