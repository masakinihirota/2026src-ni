...
### 目的
- セキュアで運用可能なアーキテクチャ基準を統一し、設計時の判断ぶれを抑える。

### 必須
- 入力検証をサーバー側で実施し、機密情報を環境変数で管理する。
- `src/proxy.ts` 運用と Edge Runtime 制約を遵守する。
- DBではRLSとSQLインジェクション対策を徹底する。
...
- **認証クライアント運用（中立表現）**:
  - 認証は **Better Auth** を正本とし、クライアントは `better-auth/react` / `better-auth/client/plugins` を使用する。
  - DBアクセスは **Drizzle** を正本とし、認証責務とデータアクセス責務を分離する。
  - 旧Supabase由来のクライアント管理記述（`createServerClient` / `createBrowserClient`）は新規実装の根拠として使用しない。
...
- **検証分離ルール**: 環境変数の検証は **build時** と **runtime時** を分離し、`next build` では `NEXT_PHASE=phase-production-build` による最小限の緩和のみ許可、実行時は fail-fast を厳格維持する。
- **IPA安全なウェブサイトの作り方**:
