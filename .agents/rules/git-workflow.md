...
### 目的
- ブランチ、コミット、PRの運用を標準化し、レビューコストと手戻りを減らす。

### 必須
- ブランチ命名規則と Conventional Commits を遵守する。
- PR本文テンプレートとチェックリストを満たす。

### 禁止
- 論理的に無関係な変更を1コミットに混在させる。
- デバッグコードやコメントアウトコードを残したままレビュー依頼する。

### 例外
- 緊急障害対応（hotfix）は最短復旧を優先し、事後に整備する。
...
| `perf` | パフォーマンス改善 |
| `revert` | コミットの取り消し |
| `hotfix` | 本番緊急対応（ブランチタイプの `hotfix` に対応） |
...
**body（任意）のルール：**
- 1行目と概要行の間に空行を1行挟む
- 「なぜこの変更が必要か」「どんな問題を解決したか」「どのように修正したか」を書く
- 必要に応じて「影響範囲（UI / API / データなど）」「追加で注意が必要な点・制約」「今後のTODO」を記載する
- 72文字で折り返す

**Issue / PR リンク（フッター）のルール：**
- 関連するIssueやPRがある場合は末尾に記載する（例: `close #123`, `ref #456`）
