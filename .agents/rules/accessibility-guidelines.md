...
### 目的
- アクセシビリティ要件を明文化し、実装とレビューの判断を揃える。

### 必須
- UIテストで `vitest-axe` による検証を行う。
- コントラスト比 4.5:1 以上を満たす。
- 最小文字サイズ 16px（1rem）以上を守る。

### 禁止
- 色だけで情報を伝える表現。
- `text-xs` / `text-sm` など 1rem 未満の文字サイズ。

### 例外
- 例外が必要な場合は、根拠と代替手段をレビュー時に明示する。

### 参照
- [UI/UXガイドライン](./ui-ux-guidelines.md)
...
- **基本サイズ**: **16px (1rem)** を基準とする（Tailwind CSS の `text-base` に相当）。
  - **最小文字サイズは 16px (1rem) を必須**とし、注釈・補助文・バッジを含めて 1rem 未満を禁止する。
  - Tailwind CSS では `text-xs` / `text-sm` を使用しない（`text-base` 以上を使用）。
- **単位**: 必ず `rem` を使用する（ブラウザ設定の実装）。
