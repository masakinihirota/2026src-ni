...
### 目的
- クリティカル領域のレビュー基準を固定し、重大な見落としを防ぐ。

### 必須
- コンポーネント分割済みのHookが存在する場合、親コンポーネントで実際に呼び出されていることをレビューで確認する。
- 起動時の環境変数検証を行う。
- RLS、インデックス、N+1対策を含むDB基準を満たす。
- クリティカル機能に対してテストを実装する。
...
function validateOAuthCredentials() {
  ...
  if (errors.length > 0) {
    ...
    ### 4.2 Hook 接続確認（分離後の回帰防止）

    **Critical**: Hook/ロジック層を分離した後に、親コンポーネントが旧実装を保持したままにならないよう、接続状況をレビューで検証する。

    **レビュー観点**:
    - ✅ `*.hook.ts` を追加・更新したPRでは、対応する `*.tsx` が `useXxxHook(...)` を呼んでいることを確認
    - ✅ 親コンポーネント内にHookと重複する `useState` / `useEffect` / handler 群が残っていないことを確認
    - ✅ Hook側API（戻り値）と表示側利用が一致し、不要なimportがないことを確認
    - ✅ Hook化で置換した場合、関連テストを再実行してUI上の機能退行がないことを確認

    **禁止**:
    - ❌ Hookを作成・更新したのに、親コンポーネントが旧ローカル状態実装を使い続ける
    - ❌ Hookと親コンポーネントで同一責務の状態管理を二重保有する
...
**Minor**: Tailwind CSS v4 の新しいクラス名を使用（ただし装飾目的のグラデーションは禁止方針を優先）
...
- ✅ 装飾目的のグラデーションは `ui-ux-guidelines.md` の禁止方針を優先する
...
**参考**:
- `.agents/rules/security-architecture.md` - セキュリティアーキテクチャ全般
- `.agents/rules/coding-standards.md` - 基本的なコーディング標準
- `.agents/skills/test-workflow/SKILL.md` - テストワークフロー
