...
TDDでテストコードと実装コードを作成する際は、次の順序を必須とします。

1. `test-case-outline-generation` で `describe` / `it.todo` の仕様アウトラインを確定
2. `test-code-generation` で構造とケース名を維持したまま `it` を実装

この2段運用を省略して直接実装しないこと。
