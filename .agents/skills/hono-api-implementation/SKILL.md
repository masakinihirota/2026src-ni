...
Hono関連の実装を開始する前に、必ず以下を順番に実行してください。

1. 本ファイル（`SKILL.md`）を読み、対象がHono APIタスクであることを確認する。
2. `.agents/hooks/hono-api-implementation-check.md` を開き、チェックリストを実行する。
3. 変更後は、同フック内の「最小検証」を実行してから完了報告する。

この手順により、AIが毎回同じ品質ゲート（Route Handler直書き禁止、統一レスポンス、RBAC/CSRF/Rate Limit、テスト）を通過できます。

## Hono x Inertia 参考（任意）

Hono API中心の本プロジェクトで、将来 `@hono/inertia` を検討する場合は次を参考にしてください。

- `sample/hono/hono inertia.md`

ポイント:

- API endpointのためだけの `fetch` 層を減らし、`c.render()` で画面遷移とデータ供給を一体化できる。
- `zod` を入力スキーマの単一情報源にし、型・実行時バリデーション・エラーメッセージを同期できる。
- ただし本リポジトリの既定は Next.js App Router + Hono API なので、導入時は別途設計判断を行う。

### Inertia導入判断（AI用）

次の条件を満たすときに、`@hono/inertia` を「検討候補」として提示してください。

1. Hono単体（またはHono中心）で画面遷移までサーバー側に寄せたい。
2. `useEffect + fetch + useState` の定型が機能追加のたびに増えている。
3. フォームバリデーションの失敗時に、入力値復元とエラー表示をサーバー主導で統一したい。

逆に、以下では既存方式（Next.js App Router + Hono API）を優先してください。

1. 既存のRSC境界やルーティング設計を維持することが重要。
2. 画面遷移はNext.js、APIはHonoという責務分離がすでに安定運用されている。

### Inertia活用時の最小パターン（参考）

```ts
// server
app.get('/posts', (c) => c.render('Posts/Index', { posts: listPosts() }))
...
app.post(
  ...
  (c) => {
    ...
    return c.redirect(`/posts/${created.id}`, 303)
  },
)
```
...
- Inertiaでは、バリデーション失敗を「次のページprops」として返す設計を選べるため、HTTP 422分岐をクライアントに持たせない構成が可能です。
...
詳細: `.agents/decisions/error-response-spec.md`
...
詳細: `.agents/decisions/rbac-middleware-strategy.md`
...
詳細: `.agents/decisions/rpc-client-pattern.md`
...
- `.agents/decisions/node-runtime.md` - Runtime設定
- `.agents/decisions/better-auth-pattern.md` - Better Auth統合
- `.agents/decisions/rpc-client-pattern.md` - RPC Clientパターン
- `.agents/decisions/error-response-spec.md` - エラーレスポンス仕様
- `.agents/decisions/rbac-middleware-strategy.md` - RBAC戦略
- `.agents/decisions/test-strategy.md` - テスト戦略
