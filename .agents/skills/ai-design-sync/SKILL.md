---
name: ai-design-sync
description: Source design documents (anti-design) and agent rules (.agents/rules) monitor and synchronize to AI-managed design documents (ai-design).
context: fork
---
...
人間が更新する `anti-design` フォルダの内容と、エージェントの最新ルール（`.agents/rules`）を常に同期し、`vns-masakinihirota-ai-design`（以下 `ai-design`）を真実のソースとして維持します。
...
1. **最優先**: `.agents/rules` および `skills` 内の制約（AIGCとしての行動規範、技術スタック）。
...
- 以下のディレクトリ内のファイルの最終更新日時（mtime）を確認し、`ai-design` 側の更新日時と比較します。
  ...
  - `u:\2026src\vns-masakinihirota.worktrees\anti\.agents\rules` (エージェントルール)
