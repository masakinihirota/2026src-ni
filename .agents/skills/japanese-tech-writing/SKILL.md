...
- 見出し・コラム見出しに、区切り線（罫線 `─` や等）で「種別──主題」「主題──概念」のように二要素を詰め込まない。見出しは単一の自然な句にする（要素を一つに絞るか、助詞・読点でつなぐ）。コラム見出しも「基礎」「補足」のような種別名だけで済ませず、「同値関係としての分類」「ループ不変条件と帰納法」のように内容を特定する。
...
- 例の節の中でも、その節の問い・帰結に関係しない過剰な詳細だけを除く。議論に必要な具体は残す。省略の典型は、エージェント報告の装飾的精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名である。
...
**重要**: このセクションは、LLMが出しやすい空疎な表現を抑止するための規範です。人間の文体規制ではなく、AI出力制御用です。
...
- このスキルは、既に下書きが存在し、推敲フェーズにあるときに適用するのが最適です。
- `writing-style` スキルで基本的な表記・文体を整えてから、本スキルで論証構造と厳密さを高めるフローがお勧めです（二段階推敲）。
- 「LLM っぽい表現の禁止」は、AI生成テキストをフィルタするための規範です。人間が執筆する場合も参考になりますが、特にAI出力の検査に効果的です。
