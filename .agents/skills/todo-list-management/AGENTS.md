...
1. **完了TODOは即時アーカイブ**
   - `schedule_todo_list/` 配下のTODOが完了（チェックボックスがすべて `[x]`）したら、
     その時点で必ず `schedule_todo_list/archive/` へ移動する。

2. **新規TODO作成時の推奨チェック**
   - 新しい計画立案依頼を受けたら、既存の `schedule_todo_list/` のアクティブTODOを確認する。
   - 既存TODOに統合できる内容なら統合を優先することを推奨する。
   - 別機能・別フェーズの場合は新規TODOファイルを作成してよい。

3. **同時アクティブTODO上限（目安）**
   - 同時にアクティブなTODOは 3 ファイルを目安とする（過剰な分散を防ぐため）。
   - 4ファイル以上になる場合は、完了・廃止できるものがないか先に確認する。
...
**Next Action**:
- [ ] スキーマ統合タスクを実施 (`.agents/decisions/schema-migration.md` 参照)
```
...
2. 実施内容を `.agents/skills/todo-list-management/IMPROVEMENTS.md` に記録
