...
### 2.1 `rg` の扱い（標準フォールバックを固定）

- `rg` が利用可能な環境では、検索速度の観点で `rg` を優先してよい。
- ただし Windows 開発環境では `rg` 未導入のケースがあるため、**`rg` が見つからない場合は即座に `Select-String` を標準フォールバックとして使う**。
- `rg` の失敗後に複数の Unix 系コマンドを試すリトライは禁止する。

**例（推奨）**

```powershell
# 文字列検索
Select-String -Path "src/**/*.ts*" -Pattern "createProfileCreationWizard"

# ファイル名検索
Get-ChildItem -Path "src" -Recurse -File -Include *.ts,*.tsx | Select-String -Pattern "ValuesDisplayPanel"
```
...
| `rg`（未導入時） | `Select-String` + `Get-ChildItem` |
