...
### 1. 失敗時の再実行禁止ルール
- 同一エラーで失敗したコマンドを、原因未解明のまま再実行してはいけない。
- 失敗したら必ず `Stop -> Cause -> Fix -> Verify` の順で進める。

`Stop -> Cause -> Fix -> Verify` の定義:
- Stop: その場で同一コマンドの再実行を止める。
- Cause: エラーログと差分（schema / migration / journal）を確認し、原因を1つ以上特定する。
- Fix: 原因に対応する修正を1つだけ適用する（同時多発修正を避ける）。
- Verify: 検証コマンドで正常化を確認してから次へ進む。

### 2. マイグレーション整合性ゲート（コミット前に必須）
- 以下の順番を固定し、途中を省略しない。
  1. `pnpm drizzle-kit generate`
  2. `pnpm db:check-migration-files`
  3. `git add -A`
  4. `git commit`
- `pnpm db:check-migration-files` が失敗した場合、コミットを継続してはいけない。
- 特に `_journal.json` に存在する tag に対応する `.sql` が無い状態を禁止する。

### 3. 失敗パターン別の即時対応ルール
- `_journal.json entries without SQL files` が出た場合:
  - `_journal.json` と `drizzle/*.sql` の対応を手動で照合する。
  - 欠損した `.sql` を復元するか、未適用エントリを削除して整合させる。
- Drizzle の対話プロンプト（rename/create 選択）が出た場合:
  - 自動で進めず、差分を確認してから1回だけ選択する。
  - 判断根拠を `docs/database/DB_ANTIPATTERN_FIXES.md` に記録する。
- データ損失警告が出た場合:
  - その場で中断し、`push` を続行しない。
  - 既存データ保全方針（バックアップ、ロールバック手順）を先に確定する。

### 4. 1変更1目的ルール
- 1回の変更で扱う論点は1つに限定する（例: FK追加のみ、NOT NULL追加のみ）。
- 複数論点を同時投入して失敗した場合は、最小単位に分割して再実行する。

### 5. 記録義務（再発防止ナレッジ化）
- 失敗を検知したら、以下を `docs/database/DB_ANTIPATTERN_FIXES.md` に追記する。
  - 発生日時
  - 失敗コマンド
  - 根本原因
  - 修正内容
  - 再発防止策
- 同じ原因の再発を検知した場合は、既存ルールを更新し、手順をより具体化する。
