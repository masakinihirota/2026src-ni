...
2. **[Boy Scout Plan]**: 今回ついでに修正する「小さな改善点」のリスト。ESLint 警告を扱う場合は、どの警告を今回返済するかを明記してください。
3. **[Execution]**: ユーザーの承認を待たず、安全な範囲（Boy Scout Plan）の修正を実行し、大きな問題についてはユーザーに判断を仰ぐ。
4. **[Warning Delta]**: 修正前後で ESLint の警告がどう増減したかを簡潔に報告してください。全件解消できない場合も、今回減らせた分を明示してください。
