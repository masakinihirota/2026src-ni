...
1.  **コンテキスト読み込み**:
    - `.agents/skills/backend-engineering/SKILL.md`を確認します。

2.  **レビュー対象**:
    - `schema.sql`, `migrations/` 以下のSQLファイル
    - `src/` 以下のPostgresクライアント使用箇所（データ取得・更新ロジック）

3.  **チェック項目**:
    - **RLS**: すべてのテーブルでRLSが有効か？ ポリシーは適切か？
    - **インデックス**: 検索条件や結合キーにインデックスが貼られているか？
    - **型安全性**: JSONBの乱用や、不適切な型キャストがないか？タイムスタンプカラムには必ず `{ withTimezone: true }` が指定されているか？
...
- `/review-db-general`: DB関連ファイルのレビュー
