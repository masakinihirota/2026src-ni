...
2.  **各コンポーネントへの適用 (Iterative Execution)**
    ...
    2.  **コードレビューの実行**:
        - 対象ディレクトリ内部に対して、以下のレビュー手順（`.agents/workflows/code-review.md` 準拠）を実行してください。
          - **機能性**: バグはないか？ 仕様通りか？
...
3.  **App層のチェック (Page Level Review)**
    - `src/app` ディレクトリ内の主要なページファイル（`page.tsx`, `layout.tsx`）をレビューします。
      ...
      - **設定漏れ**: `export const dynamic` や Metadata 設定が適切か？ 非同期リクエストAPI (`await params`) が正しく使われているか？ Parallel Routes に `default.tsx` が存在するか？
...
- **時間配分**: コンポーネント数が多い場合（10個以上）、時間がかかります。5個ごとに進捗状況をユーザーに報告してください。
