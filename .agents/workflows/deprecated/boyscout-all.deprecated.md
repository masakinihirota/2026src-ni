...
- **推奨コマンド (PowerShell)** - プロジェクトルートで実行:
  ```powershell
...
1.  **作業対象の明示**
    - 現在レビュー中のコンポーネント名をユーザーに報告します。
    - 例: `現在 'ui' コンポーネントを監査中です...`

2.  **ボーイスカウトルールの適用**
    - 以下のワークフローファイルを読み込み、指示に従って実行してください。
    - 参照ファイル: `.agents/workflows/boyscout.md`
    - 主なチェック項目:
      - 構造と配置・開発ダッシュボード登録
      - 自動化ツール (`oxlint`, `test`)
...
3.  **エラーハンドリング**
    - コンポーネント内で解決困難なエラー（型エラーやテスト落ち）が見つかった場合：
      - 無理に修正しようとせず、そのエラー内容を記録してください。
      - エラー詳細をユーザーに報告し、そのコンポーネントの作業を切り上げ、次のコンポーネントへ進んでください。
...
- **チェック項目**:
  ...
  - Parallel Routes を使用している場合、各スロットに `default.tsx` が存在し、フォールバックされているか？
...
- **時間配分**: コンポーネント数が多い場合（10個以上）、非常に時間がかかります。5個ごとに進捗状況をユーザーに報告してください。
