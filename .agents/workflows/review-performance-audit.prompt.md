...
1. コンテキストの読み込み
  - 次のスキルファイルを確認し、ベストプラクティスを理解してください: `.agents/skills/vercel-react-best-practices/SKILL.md`
...
4. 詳細監査 (Deep Dive)
   ...
   - 各コンポーネントについて以下を分析します:
     ...
     - **レンダリング**: `*.tsx` を確認し、Suspenseの使用、画像の最適化（`next/image`のデフォルトTTL変更への対応や`localPatterns`の確認など）、CWVへの配慮があるか。
     - **キャッシング**: `revalidateTag` に第2引数（cacheLife profile）が明示的に指定されているか確認。
